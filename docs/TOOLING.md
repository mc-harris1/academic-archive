...
- R: `DESCRIPTION`, `renv.lock`, `renv`, `lintr`, `styler`, `precommit`
...
- Treat `pyproject.toml` and `DESCRIPTION` as the authoritative metadata files for their respective ecosystems.
...
- Prefer `renv` restore and project-local R package state once `DESCRIPTION` and `renv.lock` exist.
...
- Root R metadata is scaffolded in `DESCRIPTION` and a seed `renv.lock` is committed for reproducibility.
